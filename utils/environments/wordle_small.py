...
class WordleSmall(gym.Env):
    ...
    def __init__(self,
                 ...
                 explore_weight_schedule=ConstantSchedule(0),
                 # PiecewiseSchedule([(0, 1), (19999, 1)], outside_value=0),
        ...
        self.valid_words = valid_words if valid_words is not None else [word.lower() for word in english_words_set if
                                                                        len(word) == self.n_letters]
        self.valid_words = [word for word in self.valid_words if
